...
const token = '5514059022:AAGNXjj-0_FZkSb5E6-hpi4lhf0JTekqeio';
...
function connectWebSocket(chatId) {
  ...
  const socket = new WebSocket('wss://soketwss.ru:3030/');
  const receivedIds = [];
  let eventData;
...
async function indexMoex() {
  const moexUrl = 'https://iss.moex.com/iss/engines/stock/markets/index/boards/SNDX/securities/imoex2.json'; 
  //const moexUrl = ' https://iss.moex.com/iss/engines/stock/markets/index/boards/SNDX/securities/IMOEX/IMOEX.json;  // Резервный адрес
...
bot.onText(/\/indexmoex/, async (msg) => {
  const chatId = msg.chat.id;
  const imageUrlmoex = 'img/moex.png';
  try {
      ...
      searchAndFormatDataByTicker(tickerToSearch)
        .then(async (foundData) => {
          if (foundData.length > 0) {
            ...
            bot.sendPhoto(chatId, `chart_screenshot_${chatId}.png`, { caption: responseMessage });
          } else {
